...
export default async function BrandDetailPage({ params }: { params: { id: string } }) {
  ...
  try {
    brand = await api(`/brands/${params.id}`, { token, cache: 'no-store', headers });
  } catch (e) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge variant={brand.isVisible ? 'success' : 'secondary'}>
              {brand.isVisible ? 'Visible' : 'Hidden'}
            </Badge>
            <Badge variant="outline">{brand._count.products} products</Badge>
          </div>
        </div>
        <CatalogueEntityActions kind="brands" id={brand.id} isVisible={brand.isVisible} />
      </div>
        ...
        <CardContent>
          <CatalogueEntityForm
            ...
            initial={{
              ...
              allBranches: brand.allBranches,
              branchIds: brand.branches.map((b) => b.branch.id),
            }}
          />
        </CardContent>
      </Card>
